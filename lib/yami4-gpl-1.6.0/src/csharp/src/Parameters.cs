...
namespace Inspirel.YAMI
{
    ...
    /// </description></item>
    /// <item><description>nested <c>Parameters</c> object, which provides 
    /// its own scope for naming.</description></item> 
    /// </list>
    /// <para> The names 
        ...
        : YAMISerializable, IEnumerable<Parameters.Entry>
    {
        ...
        /// if the given name is not found
        /// </exception>
        public void Lock(string name, long key)
        {
            Entry e = findExisting(name);
            if(e.locked)
            {
                throw new BadStateException(name);
            }

            e.locked = true;
            e.lockKey = key;
            if(e.Type == EntryType.NESTED_PARAMETERS)
            {
                lockRecursively(e, key, true);
            }
        }
        ...
        /// if the given name is not found
        /// </exception>
        public void Unlock(string name, long key)
        {
            Entry e = findExisting(name);
            if(e.locked == false)
            {
        ...
        /// </param>
        /// <returns>
        /// <c>true</c> if the entry is locked and 
        /// <c>false</c> otherwise. 
        /// </returns>
        /// <exception cref="Inspirel.YAMI.NoSuchNameException">
        /// if the given name is not found
        ...
        // helper for processing nested parameters
        // precondition: e points to nested entry
        private bool containsLocked(Entry e)
        {
            foreach(Entry child in e.GetNestedParameters())
            {
                if(child.Type == EntryType.NESTED_PARAMETERS)
                {
                    if(containsLocked(child))
                    {
                        return true;
                    }
                }
        ...
        // returns true if the entry itself is locked
        // or if it is a nested parameters that contains a locked entry
        private bool isOrContainsLocked(Entry e)
        {
            if(e.locked)
            {
                return true;
            ...
            if(e.type == EntryType.NESTED_PARAMETERS)
            {
                // nested entries need to be tested deeply
                // (recursively) to avoid replacing the whole subtree
                // that contains a nested locked entry
                if(containsLocked(e))
                {
                    return true;
                }
        ...
        // helper for locking or unlocking the whole nested hierarchy
        // locking is performed if the lock parameter is true,
        // otherwise unlocking is done, in either case existing locks or
        // locks with other key values are preserved
        // precondition: e is nested
        private void lockRecursively(Entry e, long key, bool doLock)
        {
            foreach(Entry child in e.GetNestedParameters())
            {
                if(doLock)
                {
                    // locking was requested
                ...
                {
                    // unlocking was requested
                    if(child.locked && child.lockKey == key)
                    {
                        child.locked = false;
                    }
                }
                ...
                if(child.Type == EntryType.NESTED_PARAMETERS)
                {
                    lockRecursively(child, key, doLock);
                }
            }
        }
