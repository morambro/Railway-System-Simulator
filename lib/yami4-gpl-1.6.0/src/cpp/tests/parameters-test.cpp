...
void test9()
{
    ...
    bool is_locked = params.is_locked("name");
    assert(is_locked == false);

    long long key = 1234567890LL;

    params.lock("name", key);

    is_locked = params.is_locked("name");
    assert(is_locked);
    ...
    catch (const yami::yami_logic_error & e)
    {
        // already locked
        assert(e.what() ==
            std::string("The given object is in the wrong state."));
    }

    try
    {
        // cannot modify
        params.set_integer("name", 456);
        assert(false);
    }
    catch (const yami::yami_logic_error & e)
    {
    ...
    nested.set_integer("internal", 789);

    yami::parameters nested2(
        nested.create_nested_parameters("more nested"));

    nested2.set_integer("more internal", 987);

    // lock something within the hierarchy

    long long other_key = 987654321LL;
    nested.lock("internal", other_key);

    try
    {
        nested.set_integer("internal", 123);
        assert(false);
    }
    catch (const yami::yami_logic_error & e)
    {
    ...
    params.lock("nested", key);

    try
    {
        nested2.set_integer("more internal", 123);
        assert(false);
    }
    catch (const yami::yami_logic_error & e)
    {
    ...
    nested.unlock("internal", other_key);
