 ...
 *  <li>nested <code>Parameters</code> object,
...
public final class Parameters
     ...
     * @return <code>true</code> if the entry is locked and
    ...
    private boolean containsLocked(Entry e) {
        for (Entry child : e.getNestedParameters()) {
    ...
    private boolean isOrContainsLocked(Entry e) {
        if (e.locked) {
            return true;
        }
        if (e.type == EntryType.NESTED_PARAMETERS) {
    ...
    private void lockRecursively(Entry e, long key, boolean lock) {
        for (Entry child : e.getNestedParameters()) {
            if (lock) {
            ...
            if (child.type == EntryType.NESTED_PARAMETERS) {
                lockRecursively(child, key, lock);
            }
        }
    }
