...
void test9()
{
    ...
    bool is_locked;
    res = params.is_locked("name", is_locked);
    assert(res == yami::core::ok);
    assert(is_locked == false);

    long long key = 1234567890LL;

    res = params.lock("name", key);
    assert(res == yami::core::ok);
    ...
    res = params.lock("name", key);
    assert(res == yami::core::bad_state); // already locked

    res = params.set_integer("name", 456);
    assert(res == yami::core::bad_state); // locked, cannot modify

    res = params.unlock("name", key + 1);
    assert(res == yami::core::unexpected_value); // bad key

    res = params.unlock("name", key);
    assert(res == yami::core::ok);
    ...
    res = params.unlock("name", key);
    assert(res == yami::core::bad_state); // already unlocked

    res = params.set_integer("name", 456);
    assert(res == yami::core::ok); // unlocked, can modify
    ...
    res = nested->set_integer("internal", 789);
    assert(res == yami::core::ok);
    ...
    res = nested2->set_integer("more internal", 987);
    assert(res == yami::core::ok);

    // lock something within the hierarchy

    long long other_key = 987654321LL;
    res = nested->lock("internal", other_key);
    assert(res == yami::core::ok);

    res = nested->set_integer("internal", 123);
    assert(res == yami::core::bad_state); // locked, cannot modify

    // lock the whole hierarchy except the element that is already locked

    res = params.lock("nested", key);
    assert(res == yami::core::ok);

    res = nested2->set_integer("more internal", 123);
    assert(res == yami::core::bad_state); // locked deeply, cannot modify

    res = params.set_integer("nested", 123);
    assert(res == yami::core::bad_state); // locked as a whole, cannot modify

    // unlock everything except the element that was already locked before

    res = params.unlock("nested", key);
    assert(res == yami::core::ok);
    ...
    res = nested->set_integer("internal", 123);
    assert(res == yami::core::bad_state); // still locked, cannot modify

    res = nested->unlock("internal", key);
    assert(res == yami::core::unexpected_value); // need other key

    res = nested->set_integer("internal", 123);
    assert(res == yami::core::bad_state); // still locked

    res = nested->unlock("internal", other_key);
    assert(res == yami::core::ok);

    res = nested->set_integer("internal", 123);
    assert(res == yami::core::ok); // finally unlocked

    res = params.set_integer("nested", 123);
    assert(res == yami::core::ok); // unlocked, can replace the whole
}
