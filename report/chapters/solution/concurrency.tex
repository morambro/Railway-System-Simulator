...
Internamente a ciascuna Regione di simulazione, sono definite entità che modellano le interazioni previste dalla specifica del problema. Di seguito sono descritte le principali.
	...
	In prima analisi, un Viaggiatore (\ttt{Traveler}) può essere modellato come una \ii{entità Attiva} che esegue una sequenza di operazioni semplici. In presenza tuttavia di un modello di distribuzione come quello presentato in sezione \ref{sec:distr_regioni}, si presenta il problema di modellare il passaggio da una Regione (nodo) ad un'altra del Viaggiatore, nel caso in cui il viaggio da esso percorso lo preveda.
	...
	Il cambio di Regione di un Viaggiatore potrà quindi essere ottenuto semplicemente inserendo le operazioni successive da eseguire per esso, nella coda del pool di thread del nodo destinazione.
