...
Le azioni compiute dall'operazione \ttt{LEAVE} sono:
	...
	Nel caso in cui la stazione dalla quale il Viaggiatore vuole partire sia una Stazione di Gateway, allora la procedura di accodamento diviene:
	\begin{itemize}
		\item Se la Regione di Destinazione \ttt{next\_region} è diversa dalla Regione corrente, allora
			...
			\end{itemize}
	\end{itemize}
	Questo permette di risolvere situazioni nelle quali vi siano coincidenze presso una stazione di Gateway. Si noti che l'eventuale azione di trasferimento del Viaggiatore al nuovo nodo viene effettuata dall'operazione \ttt{LEAVE} e non dalla stazione all'arrivo del Treno (descritto nella sottosezione \ref{subsubsec:regional_station_access}). Questa scelta è stata fatta per limitare il più possibile l'occupazione da parte di un Treno della Piattaforma (risorsa protetta), ed evitare quindi l'esecuzione di operazioni potenzialmente lunghe come l'invio di un messaggio remoto. Queste considerazioni hanno guidato anche la definizione delle azioni eseguite all'Arrivo a Destinazione.
