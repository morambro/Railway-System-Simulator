...
% ############################################################################################################
		...
		Sia \ttt{Access\_Number} il numero di accessi nella direzione corrente, e siano \ttt{First\_Direction\_Queue} e \ttt{Second\_Direction\_Queue} contatori del numero di Treni in attesa di accedere al Segmento per ciascuna delle due direzioni possibili.
		...
		\begin{itemize}
			...
			\item Se $T$ può accedere allora:
				...
				\end{itemize}
			\item Se $T$ non può accedere, allora a seconda della direzione con la quale intende percorrere il Segmento incrementa di $1$ il valore di \ttt{First\_Direction\_Queue} oppure \ttt{Second\_Direction\_Queue}, e accoda il thread $T$ presso la coda \ttt{Waiting\_Queue}.
		\end{itemize}
			
			Una volta che il Segmento si sarà liberato (\ttt{Free=True}) allora ciascun Treno in attesa nella coda \ttt{Waiting\_Queue} potrà ritentare l'accesso al Segmento (la coda viene svuotata e ripopolata) e, nel caso in cui possa accedere, decrementa il valore di \ttt{First\_Direction\_Queue} o di \ttt{Second\_Direction\_Queue}, a seconda della direzione di percorrenza.
