	...
	\begin{itemize}
		...
		\item Nel caso in cui si abbia una risposta positiva da una Regione $R$ allora:
			\begin{itemize}
				...
				\item Per ciascuna Regione $R_i$ viene inviata una richiesta remota \ii{sincrona} per ottenere un Ticket che collega le varie stazioni di Gateway interne: i Biglietti ottenuti una volta uniti, permettono di raggiungere \ttt{To} a partire da \ttt{From}. La lista di Ticket ottenuta viene \ii{Validata} in modo tale da penotare posti a sedere se necessario, per ciascun Biglietto.
